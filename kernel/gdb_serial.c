...
#define PUTERR(code)	do { errchar = (code); goto errpacket; } while (0)
	...
	do {
		...
		case 'P':	/* set individual CPU register */
		{
			...
			if (reg == 0x29) goto ok;
			// orig_eax (gdb : gdb/regformats/reg-i386-linux.dat)
			/*
		...
		case 'm':
		{
			...
			putpacket({(unsigned char *)inbuf, len, 1});
			break;
		}
		...
		case 'M' :
		{
...

			{
				...
				if (hex2bin(ptr2, (unsigned char *)outbuf, len))
					PUTERR(2);
		...
		case '?':
			if (cb && cb->thread_switch &&
				 ...
				 */
				char tid[2 * sizeof(unsigned long)];
				int n = 0;
				unsigned long x = (unsigned long) p->thread_id;
				do {
					tid[n++] = hexchars[x & 0xf];
					x >>= 4;
				} while (x != 0);
				memcpy(inbuf, "thread:", 7);
				x = 7;
				do {
					inbuf[x++] = tid[--n];
				} while (n > 0);
				inbuf[x++] = ';';
				inbuf[x] = 0;
				putpacket({(unsigned char *)"T"},{&p->signo,1,1},{(unsigned char *)inbuf});
			}
			else
				putpacket({(unsigned char *)"S"},{&p->signo,1,1});
			break;
		...
		case '!':
			...
			/* FALLTHROUGH */
		ok:
		putpacket({(unsigned char *)"OK"});
		break;

		errpacket:	/* error replies jump to here */
		putpacket({(unsigned char *)"E"},{&errchar,1,1});
		break;
...
void gdb_serial_exit(int exit_code)
{
	...
	putpacket({(unsigned char *)"W"},{&exchar,1,1});
	connected = 0;
...
void gdb_serial_killed(int signo)
{
	...
	/* Indicate that we have terminated.  */
	putpacket({(unsigned char *)"X"},{&exchar,1,1});
	connected = 0;
...
void gdb_serial_putchar(int ch)
{
	...
	putpacket ({(unsigned char *)"O"}, {&uch,1,1});
	base_critical_leave();
}
...
void gdb_serial_puts(const char *s)
{
	...
	putpacket({(unsigned char *)"O"}, {(unsigned char *)s,0,1}, {(unsigned char *)"\n",1,1});
	base_critical_leave();
}
