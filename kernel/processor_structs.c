...
void fill_interrupt(unsigned int vector, unsigned short selector,
                                            void *address, unsigned int DPL)
{
    unsigned long p0, p1;

    p0 = ((unsigned int)address & 0xffff) | (selector << 16);
    p1 = ((unsigned int)address & 0xffff0000) | 0x8e00 | ((DPL & 3) << 13);

    idt[vector] = (((unsigned long long)p1)<<32) | p0;
}
