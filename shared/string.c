...
/* Copy a buffer into userspace */
int memread (char* bufin, size_t sin, int offset, char* bufout, size_t len)
{
    if(len<0)       return -1;
    if(len == 0)    return 0;
    if(offset>=sin) return 0;    /* EOF */
    /* You gotta blit it before we send it */
    if(len > sin - offset)
        len = sin - offset;
    memcpy(bufout,bufin+offset,len);
    return len;
}
