 ...
 * projet.
 * Les equipes composees uniquement d'eleves en option TST ne sont pas tenues
 * d'implanter les appels systemes relatifs a l'horloge. Pour les desactiver,
 * il faut ajouter l'option : -DTELECOM_TST a la ligne DEF= du
 * Makefile du repertoire dans lequel le fichier est place.
...
unsigned long long
div64(unsigned long long num, unsigned long long div, unsigned long long *rem)
{
...
test7(void)
{
	printf("Test desactive pour les TST.\n");
}
...
proc16_1(void *arg)
{
	...
	if (screate(0) >= 0) assert(!"*** Decrease the semaphore capacity of your"
  "system to NBSEMS to pass this test. ***");
...
#if defined microblaze
static const int heap_len = 512 << 10;
#else
...
test_proc(void *arg)
{
	...
	__asm__ volatile("mfs %0,rmsr; brlid %1,0f; nop; 0: nop":"=r" (status), "=r" (ip));
	printf("RMSR = %#lx, IP = %#lx\n", status, ip);
	(void)rand;
	setSeed(ip);
#else
	unsigned long flags;
